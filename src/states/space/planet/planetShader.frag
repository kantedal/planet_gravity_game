...
void main(void) {
  ...
  vec3 ka = planetColor * 0.15;
  vec3 kd = planetColor * 1.0;
  vec3 ks = vec3(0.2);

  vec3 pos = planetPosition + vec3(planetSize - uv.x * planetSize, uv.y * planetSize, planetSize * 0.5 * (1.0 - length(abs(normal.xy))));
  planetColor = phong(ka, kd, ks, 5.0, normal, pos);
